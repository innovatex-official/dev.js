...
import { createRouteDefinition, discoverRoutes, isRouteModule, matchRoute } from "./index.js";
...
describe("matchRoute", () => {
  it("matches normalized paths", () => {
    const route = Object.freeze({ path: "/about", filePath: "/repo/app/routes/about.ts" });

    expect(matchRoute({ root: "/repo/app/routes", routes: [route] }, "/about/")).toBe(route);
  });
});
